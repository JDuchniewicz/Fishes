...
struct floe* translateToFloe(int x, int y){
    ...
    for(i = 0; i <(gameData.SizeX*gameData.SizeY); i++){
        ...
        } else{ //we give it arbitrary values to show that it is not viable to be a floe //TODO: make sure it will work, probably initalize the pointer to some weird floe
            floetrans->x = -100;
            floetrans->y = -100;
            floetrans->fishes = 0;
...
bool checkSurroundings(struct floe *ourFloe){
    ...
    for(i = 0; i < 6; i++){
        ...
        if(!currFloe->canBeEntered || !currFloe->isFloe || currFloe->penguins != 0){ //TODO: Add protection against reading the floes out of range/ boundaries etc
